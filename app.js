...
async function exportPdfFile(title) {
  ...
  tempHost.innerHTML = `
    ...
    <section class="document-shell">
      <header class="export-header">
        <h1 class="export-title">${title}</h1>
        <p class="export-meta">Generated by WanForge Markdown Renderer · ${new Date().toLocaleString('id-ID')}</p>
      </header>
      <article>${contentHtml}</article>
      <footer>© 2026 WanForge · wanforge.asia</footer>
    </section>
  `;
  ...
  try {
    const docNode = tempHost.querySelector('.document-shell');
    ...
    while (imgHeight + offsetY > usableHeight) {
